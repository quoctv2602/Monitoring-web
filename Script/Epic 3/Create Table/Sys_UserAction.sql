...
GO
/* Begin SCRIPTS*/
	IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Sys_UserAction]') AND type in (N'U'))
	BEGIN
		CREATE TABLE [dbo].[Sys_UserAction](
			[GroupId] [int] NOT NULL,
			[ActionId] [int] NOT NULL,
			[PageId] [int] NOT NULL,
			[CreatedDate] [datetime] NULL CONSTRAINT [DF_Sys_UserAction_CreatedDate]  DEFAULT (getdate()),
		 CONSTRAINT [PK_Sys_UserAction] PRIMARY KEY CLUSTERED 
		(
			[GroupId] ASC,
			[ActionId] ASC,
			[PageId] ASC
