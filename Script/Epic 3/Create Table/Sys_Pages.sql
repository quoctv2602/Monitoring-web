...
GO
/* Begin SCRIPTS*/
	IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Sys_Pages]') AND type in (N'U'))
	BEGIN
		CREATE TABLE [dbo].[Sys_Pages](
			[PageId] [int] NOT NULL,
			[PageName] [nvarchar](256) NULL,
			[CreatedDate] [datetime] NULL CONSTRAINT [DF_Sys_Pages_CreatedDate]  DEFAULT (getdate()),
		 CONSTRAINT [PK_Sys_Pages] PRIMARY KEY CLUSTERED 
		(
			[PageId] ASC
