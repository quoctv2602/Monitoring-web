...
IF EXISTS (SELECT *
FROM sys.objects
WHERE object_id = OBJECT_ID(N'[dbo].[Sys_NodeType]') AND type in (N'U'))
BEGIN
	IF(NOT EXISTS (SELECT 1 FROM Sys_NodeType AS s WHERE s.NodeType = 'Transaction'))
	BEGIN
		INSERT INTO Sys_NodeType (ID, NodeType, Description) VALUES (1, 'Transaction',null)
	END
	IF(NOT EXISTS (SELECT 1 FROM Sys_NodeType AS s WHERE s.NodeType = 'System'))
	BEGIN
		INSERT INTO Sys_NodeType (ID, NodeType, Description) VALUES (2, 'System',null)
	END
END
...
INSERT INTO [dbo].[DBScriptHistory]
    ([FileName],[Version])
VALUES('InsertData_Sys_NodeType.sql', 1);
GO
/* Tracking HISTORY */
