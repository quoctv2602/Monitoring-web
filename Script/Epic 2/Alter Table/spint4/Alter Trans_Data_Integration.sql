...
BEGIN
    IF Not EXISTS(SELECT 1 FROM sys.columns  WHERE Name = N'RowID' AND Object_ID = Object_ID(N'Trans_Data_Integration'))
	BEGIN
		ALTER TABLE Trans_Data_Integration
		ADD RowID BIGINT;
	END
END
...
IF EXISTS (  SELECT *
FROM sys.objects
WHERE object_id = OBJECT_ID(N'[history].[Trans_Data_Integration_Archive]'))
BEGIN
    IF Not EXISTS(SELECT 1 FROM sys.columns  WHERE Name = N'RowID' AND Object_ID = Object_ID(N'[history].[Trans_Data_Integration_Archive]'))
	BEGIN
		ALTER TABLE history.Trans_Data_Integration_Archive
		ADD RowID BIGINT;
	END
END
...
INSERT INTO [dbo].[DBScriptHistory]
    ([FileName],[Version])
VALUES('Alter Trans_Data_Integration.sql', 4);
GO
/* Tracking HISTORY */
