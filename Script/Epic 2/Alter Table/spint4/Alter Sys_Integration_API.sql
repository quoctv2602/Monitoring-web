...
BEGIN
    IF Not EXISTS(SELECT 1 FROM sys.columns  WHERE Name = N'IsDefaultNode' AND Object_ID = Object_ID(N'Sys_Integration_API'))
	BEGIN
		ALTER TABLE Sys_Integration_API
		ADD IsDefaultNode BIT;
	END
END
...
INSERT INTO [dbo].[DBScriptHistory]
    ([FileName],[Version])
VALUES('Alter Sys_Integration_API.sql', 4);
GO
/* Tracking HISTORY */
