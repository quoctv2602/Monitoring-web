...
FROM sys.objects
WHERE object_id = OBJECT_ID(N'[dbo].[Trans_Data_Integration_Log]') AND type in (N'U'))
BEGIN

    CREATE TABLE [dbo].[Trans_Data_Integration_Log]
    (
        ...
        CONSTRAINT [PK_Trans_Data_Integration_Log] PRIMARY KEY CLUSTERED 
    (
        [ID] ASC
    )WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
    ) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]

    ALTER TABLE [dbo].[Trans_Data_Integration_Log] ADD  CONSTRAINT [DF_Trans_Data_Integration_Log_CreatedDate]  DEFAULT (getdate()) FOR [CreatedDate]
END
...
INSERT INTO [dbo].[DBScriptHistory]
    ([FileName],[Version])
VALUES('4.Trans_Data_Integration_Log.sql', 1);
GO
/* Tracking HISTORY */
