...
FROM sys.objects
WHERE object_id = OBJECT_ID(N'[history].[Trans_Message_Log_Archive]') AND type in (N'U'))
	BEGIN
	CREATE TABLE [history].[Trans_Message_Log_Archive](
		...
		[DayofYear] [int] NULL,
		[Date] [int] NULL,
	CONSTRAINT [PK_Trans_Message_Log_Archive] PRIMARY KEY CLUSTERED 
	(
	[ID] ASC
...
INSERT INTO [dbo].[DBScriptHistory]
	([FileName],[Version])
VALUES('history.Trans_Message_Log_Archive.sql', 3);
GO
/* Tracking HISTORY */
