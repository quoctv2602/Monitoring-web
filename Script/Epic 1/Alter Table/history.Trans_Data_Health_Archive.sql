...
FROM sys.objects
WHERE object_id = OBJECT_ID(N'[history].[Trans_Data_Health_Archive]') AND type in (N'U'))
	BEGIN
	CREATE TABLE [history].[Trans_Data_Health_Archive](
		...
		[BeginTransaction] [datetime] NULL,
		[EndTransaction] [datetime] NULL,
		[BeginFileTransfer] [datetime] NULL,
		[EndFileTransfer] [datetime] NULL,
		[FileTransferViolation] [bit] NULL,
		[TransferElapsed] [int] NULL,
		[Status] [int] NULL,
		...
		[DayofYear] [int] NULL,
		[Date] [int] NULL,
		[TransactionElapsed] [int] NULL,
		[TransactionViolation] [bit] NULL,
	CONSTRAINT [PK_Trans_Data_Health_Archive] PRIMARY KEY NONCLUSTERED 
	(
	[ID] ASC
...
INSERT INTO [dbo].[DBScriptHistory]
	([FileName],[Version])
VALUES('history.Trans_Data_Health_Archive.sql', 3);
GO
/* Tracking HISTORY */
