...
WHERE object_id = OBJECT_ID(N'[dbo].[Sys_Threshold_Rule]') AND type in (N'U'))
	BEGIN
    CREATE TABLE [dbo].[Sys_Threshold_Rule]
    (
        [ID] [int] IDENTITY(1,1) NOT NULL,
        [Node_Setting] [int] NOT NULL,
        [EnvironmentID] [int] NOT NULL,
        [MachineName] [nvarchar](500) NOT NULL,
        [MonitoringType] [int] NOT NULL,
        [MonitoringName] [nvarchar](255) NULL,
        [Condition] [tinyint] NULL,
        [Threshold] [int] NULL,
        [ThresholdCounter] [int] NULL,
        [CreateDate] [datetime] NULL,
        CONSTRAINT [PK_Sys_Threshold_Rule] PRIMARY KEY CLUSTERED 
		(
			[ID] ASC
		)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
    ) ON [PRIMARY]
    ALTER TABLE [dbo].[Sys_Threshold_Rule] ADD  CONSTRAINT [DF_Sys_Threshold_Rule_CreateDate]  DEFAULT (getdate()) FOR [CreateDate]
END
