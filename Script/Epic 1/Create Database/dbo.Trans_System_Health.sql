...
WHERE object_id = OBJECT_ID(N'[dbo].[Trans_System_Health]') AND type in (N'U'))
	BEGIN
    CREATE TABLE [dbo].[Trans_System_Health]
    (
        ...
        CONSTRAINT [PK_Trans_System_Health] PRIMARY KEY CLUSTERED 
		(
			[ID] ASC
		)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
    ) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]
    ALTER TABLE [dbo].[Trans_System_Health] ADD  CONSTRAINT [DF_Trans_System_Health_CreatedDate]  DEFAULT (getdate()) FOR [CreatedDate]
END
